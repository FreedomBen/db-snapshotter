...
The Docker image is `almalinux:8.10` + the postgres-16 + mysql-8.0 client modules + the AWS CLI v2 installed from the official zip. Built and pushed to `docker.io/freedomben/db-snapshotter`.
...
- Bumping the postgres/mysql client versions is a `dnf module enable` change in the `Dockerfile`. The client version must be `>=` the server version, or `pg_dump` will refuse to run against a newer server.
